from model_stuff.models_helper import load_model, predict
import numpy as np
import os
...
def is_food(image_array):
    ...
    model_url = os.getenv("FOOD_MODEL_URL")
    # Load the model
...
def classify_food (image_array):
    ...
    # Load the model
    model = load_model(model_url)# Make a prediction
    prediction = predict(model, image_array)
    # Return whether or not the prediction is food
    ...
    return class_labels_101[np.argmax(prediction)]